...
MOCK_RESULTS = {
    ...
    "unit_converter"     : lambda args: {
        "result": round(float(args.get("value", 0)) * UNIT_DATA.get(f"{args.get('from_unit', '').lower()}_"
                                                                    f"{args.get('to_unit', '').lower()}", 1),
                        4)
    },
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5,
    ...
    for turn in range(max_turns):
        ...
        if len(new_ids) != len(new_logps): Logger(
            f"rollout token/logprob length "
            f"mismatch: {len(new_ids)} vs {len(new_logps)}"
        )
        pairs = [
            (t, lp) for t, lp in zip(new_ids, new_logps) if
    ...
    return (
        final_output, final_context, prompt_ids, response_ids, response_mask,
        response_old_logps, list(all_outputs), unfinished
    )
...
def rollout_batch(rollout_engine, tokenizer, messages_batch, tools_batch, num_gen, max_turns=3, max_new_tokens=256,
    ...
    for messages, tools in zip(messages_batch, tools_batch):
        for _ in range(num_gen):
            ...
            (
                completion, context, prompt_ids, response_ids,
    ...
    return (
        all_completions, all_contexts, all_prompt_ids, all_response_ids,
...
def validate_gt_in_text(text, gt_list):
    ...
    return {g for g in gt_list if (
            ((s := str(g).strip()) and s.lower() in text.lower())
...
def rl_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model=None, start_step=0, wandb=None,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with (torch.no_grad()):
            (
                completions, contexts, prompt_ids_batch,
                ...
                tools_batch, args.num_generations, max_turns=3,
        ...
        rewards = calculate_rewards(
            ...
            args.num_generations, reward_model,
            device=args.device,
